...
export const AV_SHAPES = { circle: "50%", round: "28%", square: "14px" };
export const FONT_STACKS = {
  sans: "var(--font-manrope),ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif",
  serif: "Georgia,'Times New Roman',Cambria,serif",
  mono: "ui-monospace,'Cascadia Mono','SF Mono',Menlo,Consolas,monospace",
};
export const GRID_MODES = { list: 1, grid2: 1 };
...
export function socIcon(kind, url) {
  ...
  return `<a class="si" href="${esc(url)}" target="_blank" rel="noopener nofollow" aria-label="media sosial"${color ? ` style="color:${color}"` : ""}><svg viewBox="0 0 24 24" fill="currentColor" width="19" height="19">${p}</svg></a>`;
}
...
export function renderPublic(u, links, opts = {}) {
  ...
  const avShape = AV_SHAPES[u.av] || AV_SHAPES.circle;
  const fontStack = FONT_STACKS[u.font] || FONT_STACKS.sans;
  const gridCss = u.grid === "grid2"
    ? "main{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px}main .lnk{padding:11px 12px}main .lnk .go{display:none}@media(max-width:420px){main{grid-template-columns:1fr!important}}"
    : "";
  ...
  const rows = main
    .map((l) => {
      const g = String(l.grp || "").trim();
      const head = g && g !== lastGrp ? `<div class="grp">${esc(g)}</div>` : "";
      lastGrp = g || null;
      const chip = l.emoji ? resolveChip(l.emoji) : "";
      return `${head}<a class="lnk" href="${esc(l.url)}" target="_blank" rel="noopener nofollow" data-id="${Number(l.id)}">
...
if(navigator.share){navigator.share({title:document.title,url:u}).catch(function(){})}
  ...
  const css = PAGE_CSS.replaceAll("{{BG}}", t.bg).replaceAll("{{DECO}}", t.deco)
    ...
    .replaceAll("{{AVR}}", avShape).replaceAll("{{FONTSTACK}}", fontStack)
    + (u.accent ? `:root{--acc:${u.accent};--glow:${u.accent}55;--chip:${u.accent}22}` : "")
    + gridCss;
...
body{font-family:{{FONTSTACK}};background:var(--bg);background-image:var(--deco);background-attachment:fixed;color:var(--fg);min-height:100vh;-webkit-font-smoothing:antialiased}
...
.av{width:96px;height:96px;border-radius:{{AVR}};background:var(--btn);border:2px solid var(--line);box-shadow:0 0 0 6px color-mix(in srgb,var(--chip) 55%,transparent),0 14px 34px -14px var(--glow);overflow:hidden;display:flex;align-items:center;justify-content:center;margin-bottom:18px}
...
.grp{grid-column:1/-1;font-size:11.5px;font-weight:800;letter-spacing:1.8px;text-transform:uppercase;color:var(--mute);text-align:left;margin:14px 2px -2px;display:flex;align-items:center;gap:10px}
.grp::after{content:"";flex:1;height:1px;background:var(--line);opacity:.7}
main .grp:first-child{margin-top:0}
