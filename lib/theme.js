...
export function socIcon(kind, url) {
  const p = ICON_PATHS[kind] || ICON_PATHS.web;
  return svg(p, url);
}
...
export function publicPageData(u, links) {
  ...
  for (const l of links) {
    ...
    if (d.kind === "mail" || ICON_PATHS[d.kind]) soc.push(d);
    else main.push(d);
  }
...
export function renderPublic(u, links, opts = {}) {
  ...
  const avatarHtml = u.avatar
    ? `<div class="av"><img src="${esc(u.avatar)}" alt="" referrerpolicy="no-referrer" onerror="this.remove();this.parentNode.classList.add('bad');this.parentNode.querySelector('.init').hidden=false"><span class="init" hidden>${esc(initials(u.name))}</span></div>`
  ...
  const rows = main.map((l) => {
    const chip = l.emoji ? `<span class="ch">${esc(l.emoji)}</span>` : "";
...
if(navigator.share){navigator.share({title:document.title,url:u}).catch(function(){})}
  ...
  const body = `<div class="wrap">
    ...
    ${socRow}
    <main>${rows}</main>
    <span class="hits">👁 ${fmtNum(u.views)} kunjungan</span>
    <div class="foot"><span>Dibuat dengan <b>🔗 Pautin</b></span><span>Bergabung ${month}</span></div>
  </div>
  <div class="sharef"><button id="sh" aria-label="Bagikan halaman ini" title="Bagikan halaman ini"><svg viewBox="0 0 24 24" width="21" height="21" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.6 13.5 6.8 4M15.4 6.5l-6.8 4"/></svg></button></div>
...
body{font-family:'Plus Jakarta Sans',ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;background:var(--bg);background-image:var(--deco);background-attachment:fixed;color:var(--fg);min-height:100vh;-webkit-font-smoothing:antialiased}
.wrap{max-width:560px;margin:0 auto;padding:56px 22px 40px;display:flex;flex-direction:column;align-items:center;text-align:center;animation:in .5s ease both}
@keyframes in{from{opacity:0;transform:translateY(10px)}to{opacity:1}}
.av{width:96px;height:96px;border-radius:50%;position:relative;background:var(--btn);border:2px solid var(--line);box-shadow:0 0 0 6px color-mix(in srgb,var(--chip) 55%,transparent),0 14px 34px -14px var(--glow);overflow:hidden;display:flex;align-items:center;justify-content:center;margin-bottom:18px}
.av img{width:100%;height:100%;object-fit:cover}
.av.bad{background:var(--chip)}
.av .init{font-size:34px;font-weight:700;letter-spacing:.5px;color:var(--fg)}
h1.name{font-size:25px;font-weight:800;letter-spacing:-.3px}
p.user{font-size:13px;color:var(--mute);margin:5px 0 6px;font-weight:500}
p.bio{font-size:14.5px;color:var(--mute);max-width:420px;line-height:1.55;margin:6px auto 0}
.soc{display:flex;gap:14px;justify-content:center;margin:22px 0 6px;flex-wrap:wrap}
.si{width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:var(--btn);border:1px solid var(--line);color:var(--fg);transition:.18s;backdrop-filter:blur(8px)}
.si:hover{transform:translateY(-3px);color:var(--acc);border-color:var(--acc);box-shadow:0 10px 24px -10px var(--glow)}
main{width:100%;margin-top:24px;display:flex;flex-direction:column;gap:12px}
.lnk{display:flex;align-items:center;gap:14px;background:var(--btn);border:1px solid var(--line);border-radius:var(--rad);padding:13px 18px;color:var(--fg);text-decoration:none;backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);transition:transform .16s,box-shadow .16s,border-color .16s;text-align:left}
...
.lnk .ch{flex:0 0 40px;width:40px;height:40px;border-radius:50%;background:var(--chip);display:flex;align-items:center;justify-content:center;font-size:19px}
.lnk .tt{flex:1;font-size:15px;font-weight:600;letter-spacing:-.1px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.lnk .go{flex:0 0 auto;width:17px;height:17px;opacity:.45}
.lnk:hover .go{opacity:1;color:var(--acc)}
.foot{font-size:12px;color:var(--mute);margin-top:44px;display:flex;flex-direction:column;align-items:center;gap:4px;opacity:.9}
.foot b{color:var(--fg);font-weight:600}
.hits{font-size:12px;color:var(--mute);margin-top:14px;display:inline-flex;align-items:center;gap:5px}
.sharef{position:fixed;bottom:22px;right:22px;z-index:9}
.sharef button{width:52px;height:52px;border-radius:50%;border:1px solid var(--line);background:var(--btn);color:var(--fg);backdrop-filter:blur(12px);cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 12px 30px -8px rgba(0,0,0,.45);transition:.2s}
.sharef button:hover{transform:translateY(-3px) scale(1.04);border-color:var(--acc)}
.toast{position:fixed;left:50%;bottom:26px;transform:translateX(-50%) translateY(80px);background:var(--fg);color:var(--bg);font-size:13.5px;font-weight:600;padding:10px 18px;border-radius:999px;transition:.3s;opacity:0;z-index:20;box-shadow:0 12px 30px rgba(0,0,0,.3);pointer-events:none}
.toast.show{transform:translateX(-50%);opacity:1}
@media(max-width:480px){.wrap{padding-top:44px}.sharef{right:16px;bottom:16px}}
`;
