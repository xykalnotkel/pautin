...
export function resetEmailHtml({ name, link }) {
  return SHELL(`
    <h1 style="margin:0 0 10px;font-size:19px;color:#16382F">Atur ulang kata sandi, ${name.split(" ")[0]}</h1>
    <p style="margin:0 0 18px;font-size:14px;color:#55635C;line-height:1.65">Kami menerima permintaan kata sandi baru untuk akun Pautin-mu. Klik tombol di bawah untuk memilih kata sandi baru:</p>
    <a href="${link}" style="display:inline-block;background:#E4572E;color:#fff;font-weight:800;font-size:14px;text-decoration:none;padding:12px 24px;border-radius:999px">Buat kata sandi baru</a>
    <p style="margin:20px 0 0;font-size:12.5px;color:#8B8575;line-height:1.7">Tombol tidak bekerja? Salin tautan ini ke browser:<br><span style="color:#0F5B4D;word-break:break-all">${link}</span></p>
    <p style="margin:16px 0 0;font-size:12.5px;color:#8B8575">Tautan berlaku 1 jam. Abaikan email ini bila kamu tidak memintanya.</p>`);
}

export function resetEmailText({ name, link }) {
  return `Hai ${name},\nAtur ulang kata sandi akun Pautin-mu lewat tautan ini:\n${link}\n\nTautan berlaku 1 jam. Abaikan bila bukan kamu.`;
}
