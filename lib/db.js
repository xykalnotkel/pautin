...
const USER_UPGRADES = [
  ...
  ["accent", "accent TEXT NOT NULL DEFAULT ''"],
  ["rtok", "rtok TEXT NOT NULL DEFAULT ''"],
  ["rexp", "rexp INTEGER NOT NULL DEFAULT 0"],
];
...
export async function migrate() {
  ...
  try {
    ...
    const lr = await db.execute({ sql: "PRAGMA table_info(links)", args: [] });
    const lhave = new Set((lr.rows || []).map((c) => String(c.name)));
    for (const [name, ddl] of LINK_UPGRADES) {
      if (lhave.has(name)) continue;
      await db.execute({ sql: `ALTER TABLE links ADD COLUMN ${ddl}`, args: [] });
    }
  } catch (e) {
